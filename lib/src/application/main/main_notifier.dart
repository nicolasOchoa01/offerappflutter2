...
class MainNotifier with ChangeNotifier {
  ...
  List<Post> _posts = []; // Lista filtrada y mostrada en el UI
  List<Post> get posts => _posts;

  List<Post> _allPosts = []; // Buffer de todos los posts cargados
  ...
  DocumentSnapshot? _lastVisiblePost; // ⚠️ Variable utilizada correctamente
  ...
  String get orderByField => _orderByField;
  bool get descending => _descending;

  // Estado de Filtro de Categoría
  String? _activeCategory; // ⚠️ Usamos esta como la única fuente de verdad para la categoría
  String? get activeCategory => _activeCategory;

  // Estado de Post Seleccionado
  ...
  MainNotifier(this._user, this._postRepository, this._authRepository) {
    // Inicializa el filtro de categoría si es necesario, si no, es null (Todos)
    ...
    _myCommentsSubscription = _postRepository
        .getCommentsForUserStream(_user.id)
        .listen((comments) {
  ...
  // --- LÓGICA DE ORDENAMIENTO Y FILTRADO EXTERNO ---
  
  // Método para Ordenar
  void setSortOption(String field, bool descending) {
    if (_orderByField != field || _descending != descending) {
      _orderByField = field;
      _descending = descending;
      // Reiniciar la paginación para aplicar el nuevo orden desde el inicio
      refreshPosts(); 
      notifyListeners();
    }
  }
  ...
  void setCategoryFilter(String category) {
    if (_activeCategory != category) {
      ...
      // Reiniciar la paginación para aplicar el nuevo filtro desde el inicio
      refreshPosts(); 
      notifyListeners();
    }
  }
  
  // --- LÓGICA DE CARGA Y PAGINACIÓN CORREGIDA ---

  // ⚠️ CORRECCIÓN: Se agrega el parámetro isRefresh con valor por defecto false
  Future<void> loadMorePosts({bool isRefresh = false}) async {
    ...
    try {
      final result = await _postRepository.getPosts(
        // ⚠️ CORRECCIÓN: Usamos 'isRefresh' para decidir si empezamos desde el inicio (null) o continuamos (_lastVisiblePost)
        lastVisible: isRefresh ? null : _lastVisiblePost, 
        // ⚠️ CORRECCIÓN: Usamos _activeCategory, el filtro definido en la clase
      ...
      // Si es una actualización, limpiamos el buffer
      if (isRefresh) {
        _allPosts = [];
        _posts = [];
        _allPostsLoaded = false; // Resetear el estado de "todo cargado"
      }

      if (newPosts.isNotEmpty) {
        _allPosts.addAll(newPosts);
        _lastVisiblePost = newLastVisible;
        _applyFilters(); // Aplicar filtros internos (búsqueda, siguiendo) a la nueva data
      } else {
  ...
  // ⚠️ CORRECCIÓN: Llama a loadMorePosts(isRefresh: true)
  void refreshPosts() {
    _posts = [];
    _allPosts = [];
  ...
  Future<void> loadUserProfile(String userId) async {
    ...
    try {
      final profileToLoad = userId == _user.id
          ? _user
          : await _authRepository.getUser(userId);
      if (profileToLoad == null) return;

      _profileUser = profileToLoad;
      notifyListeners();

      _profileUserPosts = await _postRepository.getPostsForUser(
        profileToLoad.id,
      );
      notifyListeners();

      if (profileToLoad.favorites.isNotEmpty) {
        _profileUserFavorites = await _postRepository.getFavoritePosts(
          profileToLoad.favorites,
        );
        notifyListeners();
      }

      _profileCommentsSubscription = _postRepository
          .getCommentsForUserStream(profileToLoad.id)
          .listen((comments) {
  ...
  Future<void> followUser(String followedUserId) async {
    try {
      ...
      final refreshedProfileUser = await _authRepository.getUser(
        followedUserId,
      );
  ...
  Future<void> unfollowUser(String followedUserId) async {
    try {
      await _authRepository.unfollowUser(
        followerId: _user.id,
        followingId: followedUserId,
      );
      await refreshCurrentUser();
  ...
  Future<void> updateProfileImage(File imageFile) async {
    try {
      await _authRepository.updateUserProfileImage(
        uid: _user.id,
        imageFile: imageFile,
      );
      await refreshCurrentUser();
  ...
  void _applyFilters() {
    ...
    // Filtro por Tab (Siguiendo)
    if (_selectedFeedTab == 1) {
      filteredPosts = filteredPosts
          .where((post) => _user.following.contains(post.user?.id))
          .toList();
    }
    ...
    // NOTA: El filtro de categoría (_activeCategory) y el ordenamiento 
    // (_orderByField) se aplican directamente en la consulta de Firestore 
    // dentro de loadMorePosts. Aquí solo aplicamos los filtros internos 
    // (Tab y Búsqueda) a los posts que ya fueron cargados.
